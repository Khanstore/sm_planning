...
class SMplanningSlot(models.TransientModel):
    ...
    report_for=fields.Selection([('person',"Per Person"),('site',"Per Site")],"Report For",default='person')
    report_type=fields.Selection([('report_1',"Report 1"),('report_2',"Report 2")],"Report Type",default='report_1')
    ...
    def print_report_1(self):
        return self.env.ref('sm_planning.sm_planning_partner_report_wizerd').report_action(self)
    @api.model
...
class SmSwissReport(models.AbstractModel):
    ...
    def get_data(self,res): #here res is planning.slot in the date range
        ...
        for rec in res:
            ...
            if not person_id in data['persons']:
                ...
                data['persons'][person_id][site_id][date_day]=rec

        return data
    def get_data_1(self,res): #here res is planning.slot in the date range
        data={}
        perPerson= {}
        perSite={}

        for rec in res:
            person=rec.resource_id
            site=rec.site_id

            if person not in perPerson:
                perPerson[person]={}
                perPerson[person]['site']={}
            if site not in perPerson[person]['site']:
                perPerson[person]['site'][site]=[]
            perPerson[person]['site'][site].append(rec)
        data['personal']=perPerson
        return data
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            ...
            'data1':self.get_data_1(res),
            'get_sm_format':self.get_sm_format,
